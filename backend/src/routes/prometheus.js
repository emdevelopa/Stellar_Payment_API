 ...
 *     description: Returns the current state of Prometheus metrics for the application, including granular payment processor and path payment metrics.
 *     tags: [Monitoring]
 *     responses:
 *       200:
...
router.get("/metrics", async (req, res) => {
  try {
    const [coreMetrics, processorMetrics, pathPaymentMetrics] = await Promise.all([
      register.metrics(),
